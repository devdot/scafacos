...
# add configure option --enable-opencl
AC_DEFUN_ONCE([AX_FCS_ENABLE_OPENCL_ARG],[
AC_ARG_ENABLE([opencl],
  AS_HELP_STRING([--enable-opencl],[enable OpenCL]),,[enable_opencl=no])
])

AC_DEFUN_ONCE([AX_FCS_ENABLE_OPENCL],[
if test "x$enable_opencl" != xno ; then
  AC_DEFINE(HAVE_OPENCL,1,[Define if OpenCL is enabled])
fi
AM_CONDITIONAL([ENABLE_OPENCL],[test "x$enable_opencl" = xyes])
])
