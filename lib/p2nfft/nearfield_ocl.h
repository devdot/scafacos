    ...
    )
{
  ...
  return (1.0 - fcs_erf(adist)) / dist; /* use erf instead of erfc to fix ICC performance problems */
}
    ...
    )
{
  ...
  fcs_float adist = alpha * dist;
  fcs_float erfc_part_ri = (1.0 - fcs_erf(adist)) * inv_dist; /* use erf instead of erfc to fix ICC performance problems */
